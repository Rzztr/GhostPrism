...
async function actualizarUbicacion(mac, lat, lng) {
    ...
    const { data: data, error: error } = await sbFetch(`dispositivos?mac=eq.${encodeURIComponent(mac)}`, {
        method: "PATCH",
        extraHeaders: { Prefer: "return=representation" },
        body: JSON.stringify({ lat: lat, lng: lng, ultima_conexion: new Date().toISOString() }),
    });
    if (error) {
...
async function resolverIncidencia(id) {
    if (!id) return null;
    const { data: data, error: error } = await sbFetch(`incidencias?id=eq.${id}`, {
        method: "PATCH",
        extraHeaders: { Prefer: "return=representation" },
...
async function _getUsuarioId(curp) {
    const { data: data } = await sbFetch(`usuarios?curp=eq.${encodeURIComponent(curp)}&select=id&limit=1`);
