// js/supaBaseControll.js — GhostPrism PWA
// Conexión a Supabase usando fetch() nativo — sin SDK, sin Node.js
// Tablas: usuarios, dispositivos, incidencias, historial
...
async function verTodosDispositivos() {
  ...
  if (error) { console.error('[Supabase] verTodosDispositivos:', error); return null; }
  return data;
}
...
async function verActivos() {
  ...
  if (error) { console.error('[Supabase] verActivos:', error); return null; }
  return data;
}
...
async function verDataCelular() {
  ...
  if (error) { console.error('[Supabase] verDataCelular:', error); return null; }
  return data;
}
...
async function buscarPorMAC(mac) {
  ...
  if (error) { console.error('[Supabase] buscarPorMAC:', error); return null; }
  return data?.[0] || null;
}
...
async function actualizarUbicacion(mac, lat, lng) {
  ...
  const { data, error } = await sbFetch(
    `dispositivos?mac=eq.${encodeURIComponent(mac)}`,
    {
      ...
      body: JSON.stringify({ lat, lng, ultima_conexion: new Date().toISOString() })
    }
  );
  if (error) { console.error('[Supabase] actualizarUbicacion:', error); return null; }
  return data;
}

// ─────────────────────────────────────────────
// INCIDENCIAS
// Columnas actuales: id, tipo, status, lat, lng, notas, created_at
// tipo: 'panic' | 'disconnected'
// status: 'activa' | 'resuelta'
// ─────────────────────────────────────────────

// Guarda una incidencia — ya no requiere dispositivo_id ni atendida_por
async function saveIncident(incident) {
  ...
  if (error) { console.error('[Supabase] saveIncident:', error); return null; }
  console.log('[Supabase] Incidencia guardada:', data);
  return data;
}

// Trae todas las incidencias sin join
async function verTodasIncidencias() {
  ...
  if (error) { console.error('[Supabase] verTodasIncidencias:', error); return null; }
  return data;
}

// Trae solo incidencias activas sin join
async function verIncidenciasActivas() {
  ...
  if (error) { console.error('[Supabase] verIncidenciasActivas:', error); return null; }
  return data;
}
...
async function resolverIncidencia(id) {
  ...
  if (error) { console.error('[Supabase] resolverIncidencia:', error); return null; }
  return data;
}

// ─────────────────────────────────────────────
// HISTORIAL
// accion: 'locate' | 'lock' | 'panic' | 'command'
// ─────────────────────────────────────────────
...
async function verHistorial() {
  ...
  if (error) { console.error('[Supabase] verHistorial:', error); return null; }
  return data;
}
...
async function verHistorialPorMAC(mac) {
  ...
  if (error) { console.error('[Supabase] verHistorialPorMAC:', error); return null; }
  return data;
}
...
async function registrarAccion(accion) {
  ...
  const { data, error } = await sbFetch('historial', {
    method: 'POST',
    extraHeaders: { 'Prefer': 'return=representation' },
    body: JSON.stringify({
  ...
  if (error) { console.error('[Supabase] registrarAccion:', error); return null; }
  return data;
}
